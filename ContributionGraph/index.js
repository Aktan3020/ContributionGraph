...
const sortedWeekdays = [...weekdays.slice(currentDayOfWeek), ...weekdays.slice(0, currentDayOfWeek)];
...
days.innerHTML = sortedWeekdays.map((el) => `<li>${el}</li>`).join('');
months.innerHTML = sortedMonths.map((el) => `<li>${el}</li>`).join('');
...
async function buildCells() {
    ...
    for (let i = 0; i < 358; i++) {
        ...
        div.innerHTML = `
            ...
            <div class='date'>${weekdays[date357DaysAgo.getDay()]} ${russianDate}</div>
        `;
